...
function newer (src, dst) {
  return !fs.existsSync (dst) ||
      fs.statSync(dst).mtime < fs.statSync(src).mtime;
}
...
function build (arg) {
  switch (arg) {
    case "all":
      ...
      ["leaflet", "jquery", "suncalc", "favicon", "license",
          "index", "base.css", "usage.css", "page", "tsc"].map ((item)=>{
        build(item);
      });
      break;
    case "release":
      ...
      ["favicon", "license", "index", "base.css", "usage.css", "page", "tsc"]
        .map ((item)=>{
          build(item);
        });
      hrefToUnpkg();
      break;
    case "tsc":
      if (gt.parseTsconfig ("./tsconfig.json").files.reduce((acc, item)=>{
            return acc || newer (item, path.join ("dist", "scripts", path.basename (item, ".mts") + ".mjs"));
          }, false)) {
        try {
          execSync ("npx tsc");
        } catch (err) {
