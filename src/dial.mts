...
function _makeDial(elem: HTMLElement) {
  const $e = $(elem);
  // 全体
  $e.css({
  ...
  // 諸元初期値の設定
  const center = ($e.width() as number) / 2;
  ...
  // valueを範囲内に収める。
  function getJustified(v: number) {
    if (!(minValue < v)) {
      v = minValue;
    } else if (maxValue < v) {
      v = maxValue;
  ...
  // メモリ
  function drawScale(v: number, indicator?: string) {
    let height = 30;
  ...
  for (
    let v = (Math.floor(minValue / scaleInterval) + 1) * scaleInterval;
    v < maxValue;
    v += scaleInterval
  ) {
  ...
  // スピナーとの連携
  const $spinner = $(`#${$e.attr("for") as string}`).on("change", (e) => {
    const val = getJustified(Number((e.target as HTMLInputElement).value));
    $(e.target).val(val);
    setValueTo$dial(val);
  });
  ...
  $e.on("pointerdown", (e) => {
    ...
    $tgt.on("pointermove", (ee) => {
      // 新しい値
      const newV = leftToValue(
        startX + (ee.clientX as number) - (e.clientX as number),
      );
      // 新しい位置
    ...
    e.preventDefault();
    e.target.setPointerCapture(e.pointerId as number);
    // ドラッグ終了
  })
    .on("pointerup", (e) => {
...
export default function makeDial(
  elem: any /*string | HTMLElement | JQuery<HTMLElement>*/,
) {
  const $elems = $(elem);
