...
$(document).ready(() => {
  ...
  //----------------------------------------------------------
  // 太陽に関して
  async function exec_for_sun(mode: string) {
    try {
      // フォームの値の読み取り
      const ll = D.getTargetPseudoLatLng();
      ...
      // 見かけの太陽の位置の標高を調べる
      const tip_height =
        (await M.getHeight(ll)) + formData.target_pseudo_height;
      // 日の出、日の入りの情報の取得。
      ...
      for (let i = 0; i < timing.length; i++) {
        ...
        const res = await M.getShadow(
          ...
          tip_height,
          pos,
          formData.camera_height,
          formData.far_distance * 1000,
          (p: number) => {
  ...
  //----------------------------------------------------------
  // 月に関して
  async function exec_for_moon(mode: string) {
    try {
      // フォームの値の読み取り
      const ll = D.getTargetPseudoLatLng();
      ...
      for (let i = 0; i < timing.length; i++) {
        ...
        pos.altitude = Math.atan(
          (pos.distance * Math.sin(pos.altitude) - tip_height) /
